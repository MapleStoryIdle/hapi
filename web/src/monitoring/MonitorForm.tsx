    ...
    return (
        <>
            <form onSubmit={save} className="space-y-4" noValidate>
                ...
                <section className="rounded-[22px] border border-[var(--app-border)] bg-[var(--app-bg)] p-4 shadow-[0_1px_4px_rgba(0,0,0,0.03)]"><h2 className="text-sm font-semibold text-[var(--app-fg)]">{props.t('monitors.form.presetPrompt')}</h2><label className="mt-3 block text-xs font-medium text-[var(--app-hint)]">{props.t('monitors.form.prompt')}<textarea value={draft.prompt} onChange={(event) => update('prompt', event.target.value)} rows={5} maxLength={8000} className="ios-form-control mt-1 w-full resize-y px-3 py-2 text-sm leading-6" /></label></section>
