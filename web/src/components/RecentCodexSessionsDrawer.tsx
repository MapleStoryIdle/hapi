    ...
    return (
        <Dialog.Root open={props.open} onOpenChange={props.onOpenChange}>
            <Dialog.Portal>
                ...
                <Dialog.Content
                    className="fixed inset-y-0 left-0 z-50 flex w-[min(24rem,80vw)] max-w-none flex-col border-r border-[var(--app-border)] bg-[var(--app-bg)] pb-[env(safe-area-inset-bottom)] pt-[env(safe-area-inset-top)] shadow-[18px_0_50px_rgba(15,23,42,0.16)] outline-none"
                    aria-describedby={undefined}
                >
                    <Dialog.Title className="sr-only">{t('recentCodex.title')}</Dialog.Title>
                    <Dialog.Close
                        className="absolute right-3 top-[calc(env(safe-area-inset-top)+12px)] z-10 flex h-8 w-8 items-center justify-center rounded-full text-[var(--app-hint)] transition-colors hover:bg-[var(--app-subtle-bg)] hover:text-[var(--app-fg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-link)]"
