...
async function readJson(request: Request, limit = 32768): Promise<unknown> {
    ...
    try {
        return await Promise.race([
            ...
            new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error('Request timed out')), 5000) })
        ])
    } finally {
...
export function createMonitorWebhookRoutes(getService: () => MonitoringService | null, store: Store): Hono {
    ...
    app.post('/events', async c => {
        ...
        attempt.count++
        if (inflight >= 16 || (inflightByMonitor.get(monitor.id) ?? 0) >= 2 || attempt.count > 60) { c.header('Retry-After', '60'); return c.json({ error: 'Service busy' }, 429) }
        inflight++
...
export function createMonitorRoutes(store: Store, getEngine: () => SyncEngine | null, getService: () => MonitoringService | null): Hono<WebAppEnv> {
    ...
    app.use('/monitors/*', async (c, next) => { c.header('Cache-Control', 'no-store'); await next() })
    ...
    const validateConfig = async (config: MonitorConfig, namespace: string): Promise<string | null> => {
        ...
        if (config.schedule) {
            try { nextMonitorRun(config.schedule, Date.now()) } catch { return 'Invalid schedule, timezone, or five-field cron expression' }
        }
        if (config.request) {
            try { validateProbeRequest(config.request) } catch (e) { return e instanceof Error ? e.message : 'Invalid request' }
        }
        ...
        } catch { return 'Unable to verify directory on the selected Runner' }
        return null
    }

    app.get('/monitors', c => { c.header('Cache-Control', 'no-store'); return c.json({ monitors: store.monitors.list(c.get('namespace')) }) })
    ...
    app.post('/monitors/parse-curl', async c => {
        try {
            const body = z.object({ curl: z.string().max(16000) }).strict().parse(await readJson(c.req.raw))
            return c.json({ request: parseMonitorCurl(body.curl) })
        } catch { return c.json({ error: 'Unsupported curl. Use one HTTP(S) URL, method, headers and inline body; shell commands and file reads are not supported.' }, 400) }
    })
    app.post('/monitors', async c => {
        try {
            ...
            c.header('Cache-Control', 'no-store')
            return c.json({ monitor: store.monitors.detail(created.id, c.get('namespace')), token: created.token }, 201)
        } catch (error) {
            return c.json({ error: error instanceof Error && error.message === 'Monitor limit reached' ? error.message : 'Unable to save monitor configuration' }, 400)
        }
    })
    ...
    app.patch('/monitors/:id', async c => {
        ...
        try {
            ...
            return c.json({ monitor: store.monitors.detail(monitor.id, monitor.namespace) })
        } catch { return c.json({ error: 'Invalid configuration' }, 400) }
    })
    app.post('/monitors/:id/token', c => {
        const monitor = store.monitors.get(c.req.param('id'), c.get('namespace'))
        if (!monitor) return c.json({ error: 'Monitor not found' }, 404)
        if (monitor.config.kind !== 'webhook') return c.json({ error: 'Only webhook monitors have tokens' }, 400)
        return c.json({ token: store.monitors.rotateToken(monitor.id, monitor.namespace) })
    })
    app.get('/monitors/:id/token', c => {
        const monitor = store.monitors.get(c.req.param('id'), c.get('namespace'))
        if (!monitor) return c.json({ error: 'Monitor not found' }, 404)
        if (monitor.config.kind !== 'webhook') return c.json({ error: 'Only webhook monitors have tokens' }, 400)
        try { return c.json({ token: store.monitors.readToken(monitor.id, monitor.namespace) }) }
        catch { return c.json({ error: 'Monitor token key unavailable' }, 503) }
    })
    ...
    app.post('/monitors/:id/incidents/:incidentId/approve', async c => {
        ...
        if (!monitor) return c.json({ error: 'Monitor not found' }, 404)
        try {
            const body = z.object({ planHash: z.string().regex(/^[a-f0-9]{64}$/) }).strict().parse(await readJson(c.req.raw))
            const accepted = getService()?.approve(monitor, c.req.param('incidentId'), body.planHash)
            return accepted ? c.json({ accepted: true }, 202) : c.json({ error: 'The plan changed, was already handled, or the rule is unavailable. Refresh before confirming.' }, 409)
        } catch { return c.json({ error: 'Invalid plan confirmation' }, 400) }
    })
