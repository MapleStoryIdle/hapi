...
describe('Kanban order API', () => {
    it('isolates namespaces, synchronizes all device subscriptions, and rejects stale edits', async () => {
        ...
        expect((await request({ revision: 0, order: ['recent', 'pinned', 'unviewed', 'pending'] })).status).toBe(200)
        expect(store.kanbanOrder.get('alice').order).toEqual(['recent', 'pinned', 'unviewed', 'pending'])
        expect(store.kanbanOrder.get('bob').order).toEqual(['pending', 'unviewed', 'pinned', 'recent'])
        ...
        expect(store.kanbanOrder.get('alice').order).toEqual(['pending', 'unviewed', 'pinned', 'recent'])
    })
    it('retains empty groups, filters unknown IDs, and handles every group disappearing', async () => {
        ...
        await request({ revision: initial.revision, order: ['recent', 'pending', 'unviewed', 'pinned', `custom:${occupied.id}`, 'custom:deleted'] })
    ...
    it('migrates v24 and persists order across restarts; deleted groups cannot return', () => {
        ...
        try {
            ...
            store.kanbanOrder.set('alice', { revision: 0, order: ['recent', 'pending', 'unviewed', 'pinned', `custom:${group.id}`] })
            store.close()
            store = new Store(path)
            ...
            expect(current.order).toEqual(['recent', 'pending', 'unviewed', 'pinned'])
