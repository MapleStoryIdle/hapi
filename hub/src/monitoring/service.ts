...
export function extractMonitorPlan(messages: Pick<StoredMessage, 'content'>[]): { text: string | null; completed: boolean; failed: boolean } {
    let text: string | null = null, completed = false, failed = false
    for (const message of messages) {
        const wrapped = unwrapRoleWrappedRecordEnvelope(message.content)
        if (wrapped?.role === 'user') { text = null; completed = false; failed = false; continue }
        ...
        if (content.type === 'output' && data.type === 'assistant') {
            ...
            if (Array.isArray(assistant.content)) {
                ...
                if (blocks.length) { text = blocks.map(b => (b as { text: string }).text).join('\n'); completed = false }
            }
        }
        if (content.type === 'output' && data.type === 'result') { completed = data.subtype === 'success' && data.is_error !== true; failed = !completed }
        if (data.type === 'error' || data.type === 'turn_aborted') { completed = false; text = null; failed = true }
    }
...
function assertUnchangedBoundPlan(event: StoredMonitorIncident, messages: Pick<StoredMessage, 'content'>[]): void {
    const lastUser = [...messages].reverse().map(message => unwrapRoleWrappedRecordEnvelope(message.content)).find(message => message?.role === 'user')
...
export class MonitoringService {
    ...
    private notify(monitor: StoredMonitor, body: string): void {
        ...
        void this.push.sendToNamespace(monitor.namespace, {
            title: 'SHAPI · ' + monitor.config.name,
            body, tag: `monitor:${monitor.id}`,
            data: { type: 'monitor', url: `/monitors/${monitor.id}` }
        }).catch(() => { console.warn('[Monitoring] Push delivery failed') })
    }
    ...
    tick(): Promise<void> {
        ...
        this.ticking = this.runTick().catch(() => console.warn('[Monitoring] Background cycle failed')).finally(() => { this.ticking = null })
        return this.ticking
    }

    private async runTick(): Promise<void> {
        const now = Date.now()
        if (now - this.lastPruned >= 3600_000) { this.store.monitors.prune(now); this.lastPruned = now }
        ...
        for (const item of pending) {
            ...
            if (!this.getEngine()?.getOnlineMachinesByNamespace(monitor.namespace).some(m => m.id === event.config.machineId)) continue
            ...
            if (target) {
                const sameSource = (candidate: typeof item) => candidate.monitor.namespace === monitor.namespace
                    && candidate.event.config.machineId === event.config.machineId
                    && candidate.event.config.targetSession?.type === target.type
                    && candidate.event.config.targetSession.sessionId === target.sessionId
                if (selected.some(sameSource)
                    || pending.some(candidate => candidate.event.id !== event.id && sameSource(candidate) && ['starting', 'investigating', 'review', 'repair_queued', 'repair_starting', 'repairing'].includes(candidate.event.state))) continue
            }
            selected.push(item)
            total++
    ...
    private async dispatch(monitor: StoredMonitor, event: StoredMonitorIncident): Promise<void> {
        ...
        try {
            ...
            const result = config.targetSession ? { type: 'success' as const, sessionId: config.targetSession.sessionId } : await engine.spawnSession(config.machineId, config.directory, config.agent,
                config.model || undefined, config.reasoningEffort || undefined, false, 'simple', undefined, undefined, undefined,
                repairing ? config.permissionMode : config.agent === 'codex' ? 'read-only' : 'plan', undefined, undefined, false)
            ...
            const text = `[SHAPI monitor ${event.id}:${repairing ? 'repair' : 'investigate'}]\n` + (repairing
            ...
            if (native) {
                ...
                if (repairing) {
                    ...
                    assertUnchangedBoundPlan(event, snapshot.snapshot.data.importedMessages.map(content => ({ content })))
                }
                ...
                if (!repairing) {
                    ...
                    const staged = await engine.stageNativeKanbanFeedback(config.machineId, {
                        artifactId: event.id.replace(/-/g, ''), codexSessionId: result.sessionId, filename: 'monitor-event.md', size: bytes.length, sha256, bytes
                    })
            ...
            this.store.monitors.transition(event.id, claimedState, 'needs_attention', { error: error instanceof Error ? error.message : 'Dispatch could not be confirmed' })
    ...
    private onSyncEvent(event: SyncEvent): void {
        ...
        for (const monitor of this.store.monitors.all()) {
            ...
            else this.store.monitors.transition(incident.id, incident.state, 'needs_attention', { error: 'Session stopped before completing. Open the session to inspect it.' })
        }
    }
    ...
    private async reconcileNative(monitor: StoredMonitor, incident: StoredMonitorIncident): Promise<void> {
        ...
        try {
            ...
            if (messages.slice(start + 1).some(message => message.role === 'user')) {
                this.store.monitors.transition(incident.id, incident.state, 'needs_attention', { error: 'Another prompt followed this task. Open the source session to inspect its result.' })
                return
            }
            const output = extractMonitorPlan(messages.slice(start).map(content => ({ content })))
            this.finishReconcile(monitor, incident, id, output)
        } catch { this.reconcileUnavailable(incident) }
    }
    ...
    private reconcile(monitor: StoredMonitor, incident: StoredMonitorIncident): void {
        ...
        if (!session) { this.reconcileUnavailable(incident); return }
        ...
        if (incident.config.targetSession) {
            const lastUser = [...history].reverse().map(message => unwrapRoleWrappedRecordEnvelope(message.content)).find(message => message?.role === 'user')
            const content = lastUser?.content
            ...
            if (!isObject(content) || typeof content.text !== 'string' || !content.text.startsWith(marker + '\n')) {
                this.store.monitors.transition(incident.id, incident.state, 'needs_attention', { error: 'Cannot match the final response to this monitor task. Inspect the source session.' })
                return
            }
        }
    ...
    private finishReconcile(monitor: StoredMonitor, incident: StoredMonitorIncident, id: string, output: ReturnType<typeof extractMonitorPlan>, active = true): void {
        const repairing = incident.state === 'repairing'
        if (output.failed) {
            this.store.monitors.transition(incident.id, incident.state, 'needs_attention', { error: 'The session failed or was interrupted. Open it to inspect the result.' })
            return
        }
        ...
        if (!output.completed) {
            if (!active) this.store.monitors.transition(incident.id, incident.state, 'needs_attention', { error: 'Session disconnected without a confirmed result. Open it before retrying.' })
            return
        }
        if (!output.text) {
            this.store.monitors.transition(incident.id, incident.state, 'needs_attention', { error: 'The session completed without a reviewable final response. Open the session to inspect it.' })
            return
        }
        if (repairing) {
            ...
            this.store.monitors.transition(incident.id, 'investigating', 'needs_attention', { error: 'The plan is too long to confirm safely here. Open the session and request a shorter final plan.' })
        } else if (this.store.monitors.transition(incident.id, 'investigating', 'review', { plan: output.text, planHash: monitorPlanHash(output.text, monitorApprovalContext(incident.config), id) })) {
