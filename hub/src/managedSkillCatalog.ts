...
/** Hub-owned canonical library. Content updates require only a Hub/Web release. */
export const MANAGED_SKILL_LIBRARY: readonly ManagedSkillDefinition[] = [{
    id: 'public-share',
    name: 'Public Share',
    description: 'Create or revoke an expiring SHAPI public link, including feedback-enabled Kanban tasks.',
    version: '1.0.0',
    minimumRunnerVersion: '1.1.0',
    content: `---
name: public-share
...
export function getManagedSkillDefinition(id: string): ManagedSkillDefinition | null {
    return MANAGED_SKILL_LIBRARY.find((skill) => skill.id === id) ?? null
}
